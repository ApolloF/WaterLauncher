...
- Desktop mode for mouse and keyboard, and (from v0.3) a big picture mode for controllers with three layouts to choose from.
...
The backend lives in `internal/`: `scan` (sources, unofficial-copy detection, executable picking), `identify` (Ludusavi matching), `library` (the game library), `settings`, `app` (services the interface calls), plus small helpers (`lnk`, `vdf`, `platform`, `logx`). The Svelte 5 interface is in `frontend/src`.
